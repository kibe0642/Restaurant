...
func GetInvoice() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		defer cancel()
		if err != nil {
			c.JSON(http.StatusInternalServerError, gin.H{"error": "error while listing invoice item"})
		}
		...
		invoiceView.Payment_due = allOrderItems[0]["payment due"]
		invoiceView.Table_number = allOrderItems[0]["table number"]
		invoiceView.Order_details = allOrderItems[0]["order details"]
